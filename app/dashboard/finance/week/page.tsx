...
import { loadShifts, loadModels, loadPairs, getSetting, setSetting, FINANCE_COURSE_KEY, type PairRecord } from '@/lib/crmDb'
...
type ModelStored = { id: string; fullName: string }

function getShiftDate(shift: ShiftStored): string | null {
...
export default function FinanceWeekPage() {
  ...
  const [shifts, setShifts] = useState<ShiftStored[]>([])
  const [models, setModels] = useState<ModelStored[]>([])
  ...
  const refreshData = async () => {
    ...
    setShifts(shiftList as unknown as ShiftStored[])
    setModels(modelList.map((m) => ({ id: m.id, fullName: m.fullName })))
    setPairs(pairList)
  }
  ...
  const rows = useMemo(() => {
    ...
    const modelById = new Map(models.map((m) => [String(m.id), m.fullName]))
    for (const s of inRange) {
      const check = s.check != null && String(s.check).trim() !== '' ? Number(String(s.check).replace(/\s/g, '')) || 0 : 0
      ...
      for (const mid of modelIds) {
        ...
        if (prev) {
          prev.earnings += check
          if (isPair) prev.earningsPair += check
          else prev.earningsSingle += check
        } else {
          byModel.set(mid, {
            modelId: mid,
            fullName: name,
            earnings: check,
            earningsSingle: isPair ? 0 : check,
            earningsPair: isPair ? check : 0,
          })
        }
      }
    }
    return Array.from(byModel.values()).sort((a, b) => a.fullName.localeCompare(b.fullName))
  }, [shifts, models, pairs, appliedFrom, appliedTo])
  ...
  const totalUsdShifts = useMemo(() => {
    const parseCheck = (s: ShiftStored) =>
      s.check != null && String(s.check).trim() !== '' ? Number(String(s.check).replace(/\s/g, '')) || 0 : 0
  ...
  return (
    <div className="p-8">
      ...
      <div className="mt-2 flex flex-wrap items-center gap-6">
        <h1 className="text-2xl font-semibold text-white">Отчет неделя</h1>
      ...
      <p className="mt-1 text-zinc-400">
        {rows.length === 0 ? 'Нет данных за неделю' : `Записей: ${rows.length}`}
      </p>
                  ...
                  return (
                    <tr key={i} className="border-b border-white/5 text-zinc-200">
                      <td className="w-[50%] px-4 py-3 font-medium text-white">
                        <Link href={`/dashboard/finance/week/model/${row.modelId}?from=${appliedFrom}&to=${appliedTo}`} className="text-emerald-400 hover:underline">
                          {row.fullName}
                        </Link>
                      </td>
                      <td className="w-[25%] px-4 py-3">{totalUsd != null ? `${Number(totalUsd).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} $` : '—'}</td>
                      <td className="w-[25%] px-4 py-3">{salaryRub != null ? `${salaryRub.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ₽` : '—'}</td>
                    </tr>
                  )
                })}
