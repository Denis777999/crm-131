/**
 * CRM data layer: при входе все данные хранятся и загружаются из Supabase.
 * localStorage используется только без сессии или как кэш после успешной записи в Supabase.
...
/** Email владельца CRM — всегда полный доступ, независимо от crm_operator_accounts. */
const OWNER_EMAIL = 'anglijskogoucitel2@gmail.com'

/** Для операторов возвращает tenant_user_id, иначе auth.uid(). Владелец (OWNER_EMAIL) всегда получает auth.uid(). */
...
/** Для UI: если текущий пользователь — оператор, возвращает его имя для режима оператора. Владелец (OWNER_EMAIL) всегда null = полное меню. */
  ...
  if (supabase && userId) {
    ...
    if (!delError) {
      if (list.length > 0) {
        ...
        if (!insertError) writeLocal(OPERATORS_STORAGE_KEY, JSON.stringify(list))
        else console.warn('[crmDb] saveOperators: Supabase insert failed', insertError.message)
      } else {
...
export async function getResponsibleOperatorId(): Promise<string | null> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { data, error } = await supabase.from('crm_responsible').select('operator_id').eq('user_id', userId).maybeSingle()
...
export async function setResponsibleOperatorId(id: string | null): Promise<void> {
  ...
  if (supabase && userId) {
    if (id === null) {
      const { error } = await supabase.from('crm_responsible').delete().eq('user_id', userId)
      if (!error && typeof window !== 'undefined') localStorage.removeItem(RESPONSIBLE_OPERATOR_ID_KEY)
    } else {
      const { error } = await supabase.from('crm_responsible').upsert({ user_id: userId, operator_id: id }, { onConflict: 'user_id' })
...
export async function saveOperatorPhotos(operatorId: string, urls: string[]): Promise<void> {
  ...
  if (supabase && userId) {
    ...
    if (!error) writeLocal(OPERATOR_PHOTOS_STORAGE_KEY(operatorId), JSON.stringify(urls))
  } else {
...
export async function saveModels(list: Omit<ModelRow, 'number'>[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_models').delete().eq('user_id', userId)
    if (!delError) {
      if (list.length > 0) {
        ...
        if (!insertError) writeLocal(MODELS_STORAGE_KEY, JSON.stringify(list))
      } else {
...
export async function saveModelAccesses(modelId: string, items: SiteAccessItem[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_model_accesses').delete().eq('model_id', modelId).eq('user_id', userId)
    if (!delError) {
      if (items.length > 0) {
        const { error: insertError } = await supabase.from('crm_model_accesses').insert(
...
export async function saveModelComments(modelId: string, comments: CommentItem[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_model_comments').delete().eq('model_id', modelId).eq('user_id', userId)
    if (!delError) {
      if (comments.length > 0) {
        const { error: insertError } = await supabase.from('crm_model_comments').insert(
...
export async function loadPairs(): Promise<PairRecord[]> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { data, error } = await supabase.from('crm_pairs').select('id, model_ids').eq('user_id', userId)
...
export async function savePairs(pairs: PairRecord[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_pairs').delete().eq('user_id', userId)
    if (!delError) {
      if (pairs.length > 0) {
        const { error: insertError } = await supabase.from('crm_pairs').insert(pairs.map((p) => ({ id: p.id, user_id: userId, model_ids: p.modelIds })))
        if (!insertError) writeLocal(PAIRS_STORAGE_KEY, JSON.stringify(pairs))
      } else {
...
export async function savePairInfo(pairId: string, info: { status: string }): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error } = await supabase.from('crm_pair_info').upsert(
...
export async function savePairAccesses(pairId: string, items: SiteAccessItem[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_pair_accesses').delete().eq('pair_id', pairId).eq('user_id', userId)
    if (!delError) {
      if (items.length > 0) {
        const { error: insertError } = await supabase.from('crm_pair_accesses').insert(
...
export async function savePairComments(pairId: string, comments: CommentItem[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_pair_comments').delete().eq('pair_id', pairId).eq('user_id', userId)
    if (!delError) {
      if (comments.length > 0) {
        const { error: insertError } = await supabase.from('crm_pair_comments').insert(
...
export async function loadShifts(): Promise<ShiftRow[]> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { data, error } = await supabase.from('crm_shifts').select('*').eq('user_id', userId)
    if (!error && Array.isArray(data)) {
...
export async function saveShifts(shifts: ShiftRow[]): Promise<void> {
  ...
  const supabase = getSupabase()
  if (supabase && userId) {
    const { error: delError } = await supabase.from('crm_shifts').delete().eq('user_id', userId)
    if (!delError) {
      if (shifts.length > 0) {
        const { error: insertError } = await supabase.from('crm_shifts').insert(
          shifts.map((s) => ({
            id: s.id,
            user_id: userId,
            model_id: s.modelId,
        ...
        if (!insertError) writeLocal(SHIFTS_STORAGE_KEY, JSON.stringify(shifts))
      } else {
...
export async function setSetting(key: string, value: string): Promise<void> {
  ...
  if (supabase && userId) {
    ...
    if (!error && typeof window !== 'undefined') localStorage.setItem(key, value)
  } else if (typeof window !== 'undefined') {
    localStorage.setItem(key, value)
  }
}
